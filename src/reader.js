...
// ── Chapter content — 10 pages (≤2 short paragraphs each) ────────
const PAGES = [

  /* ── Page 1 ── Header + drop cap */
  ...
  /* ── Page 2 ── */
  `<p class="book-p book-p--first">Si uno congela esa escena y la mira sin contexto, la conclusión parece obvia: exceso de pantalla, falta de control, un error de crianza. Nosotros también crecimos escuchando eso.</p>
  <p class="book-p">Durante mucho tiempo, la conversación sobre tecnología y niños ha estado dominada por una idea muy simple: menos pantalla es mejor. En la práctica, esa idea suele simplificar demasiado un problema mucho más profundo.</p>`,

  /* ── Page 3 ── Pivot */
  `<p class="book-pivot">El punto no es la pantalla.<br>Nunca lo ha sido.</p>
  <p class="book-p book-p--first">El punto es lo que pasa dentro de ella.</p>`,

  /* ── Page 4 ── */
  `<p class="book-p book-p--first">Recuerdo una mañana cualquiera, de esas que no parecen importantes en el momento pero que terminan marcando un antes y un después. Estábamos desayunando y Amelia empezó a contarnos sobre una actualización de un videojuego. Hablaba rápido, emocionada, pero no era solo emoción. Había estructura en lo que decía.</p>`,

  /* ── Page 5 ── */
  `<p class="book-p book-p--first">Hablaba de probabilidades, de personajes que aparecían con más frecuencia ciertos días, de estrategias para avanzar más rápido. Dijo algo que parecía una frase más dentro de su entusiasmo: que ya sabía que lo iba a lograr. No lo dijo como quien desea algo. Lo dijo como quien ya hizo las cuentas.</p>`,

  /* ── Page 6 ── */
  `<p class="book-p book-p--first">Había estimado cuánto tiempo le tomaría, cuántos intentos necesitaría, qué variables podían jugar a favor o en contra. Incluso comparó con eventos anteriores del mismo juego para ajustar su expectativa. Días después, lo logró.</p>
  <p class="book-p">No es que uno tenga razón y el otro no. Lo interesante es lo que hay detrás de esas dos formas de pensar.</p>`,

  /* ── Page 7 — GATE fires here (index 6, 70% of 10) ── */
  `<p class="book-p book-p--first">Ahí fue cuando entendimos algo que después se nos fue haciendo cada vez más evidente: lo que estaba aprendiendo no tenía nada que ver con el juego en sí. Tenía que ver con cómo pensaba. Y ese tipo de pensamiento no aparece de la nada. Se construye.</p>`,

  /* ── Page 8 ── */
  `<p class="book-p book-p--first">Porque mientras desde afuera alguien puede ver a una niña "jugando", por dentro están pasando muchas más cosas. Está evaluando opciones, tomando decisiones, probando estrategias, tolerando frustración. Está, en el sentido más literal, aprendiendo a aprender.</p>
  ...
  <p class="book-p book-p--first">Porque cuando creemos que el problema es el tiempo, respondemos con control. Y con eso, a veces, cortamos procesos que podrían convertirse en algo mucho más grande.</p>`,
...
async function submitEmail(email) {
  try {
    const res = await fetch('/api/capture-email', {
      ...
      body: JSON.stringify({ email, source: 'landing_book', timestamp: new Date().toISOString() }),
    });
    return res.ok;
  } catch {
    return true;
...
function renderDots(activeIndex) {
  const container = document.getElementById('rws-dots');
  if (!container) return;
  container.innerHTML = PAGES.map((_, i) => `
    <button
      class="reader-dot${i === activeIndex ? ' is-active' : ''}"
      data-page="${i}"
      aria-label="Página ${i + 1}"
      ${i === activeIndex ? 'aria-current="true"' : ''}
    ></button>
  `).join('');
  container.querySelectorAll('.reader-dot').forEach((btn) => {
...
export function openReader() {
  ...
  const hint = document.getElementById('rws-swipe-hint');
  if (hint) {
...
function renderPage(index, direction = 'next', instant = false) {
  ...
  if (existingPages.length > 0 && !instant) {
    ...
    const fallbackTimer = setTimeout(() => { if (pageToExit.parentNode) pageToExit.remove(); }, 450);
...
function buildModals() {
    ...
    <div class="modal-email">
      ...
      <form id="rws-email-form" class="modal-email__form" novalidate>
        <div class="modal-email__field">
          <input type="email" id="rws-email-input" name="email" placeholder="tu@email.com"
            autocomplete="email" required aria-label="Tu correo electrónico"/>
        </div>
  ...
  <div id="rws-modal-reader" class="modal-overlay modal-overlay--reader" role="dialog" aria-modal="true" aria-label="Lector del libro">
    <div class="reader-shell">
      ...
      <div class="reader-body">
        ...
        <div class="reader-gate" id="rws-gate" aria-live="polite">
          <div class="reader-gate__card">
            ...
            <button class="reader-gate__btn-secondary" id="rws-gate-continue">Seguir leyendo</button>
          </div>
        </div>
      </div>

      <!-- Swipe hint (mobile) -->
      <div class="reader-swipe-hint" id="rws-swipe-hint" aria-hidden="true">
      ...
      <!-- Dot navigation -->
      <div class="reader-dots" id="rws-dots" aria-label="Páginas"></div>
...
function bindEvents() {
  ...
  document.getElementById('rws-gate-amazon').addEventListener('click', () => track('click_amazon', { source: 'gate' }));
  document.getElementById('rws-gate-continue').addEventListener('click', dismissGate);

  document.addEventListener('keydown', (e) => {
    if (!document.getElementById('rws-modal-reader').classList.contains('is-open')) return;
